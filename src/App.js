...
import { CONTENT_VIEW_QUERY } from "./queries";
import { gql, useQuery } from '@apollo/client';
function App() {
  const { loading, error, data } = useQuery(CONTENT_VIEW_QUERY, {
    variables: {
      "first": 200,
      "skip": 0,
...
useEffect(()=>{
  if(data){
    Mapdatatoallgames(data)
  }
},[data])

function Mapdatatoallgames(gameslist) {
  let gamedata = gameslist.content.edges
  let newGamelist = []
  gamedata.map((item,index)=>{
    console.log(item.node.footage.split(";"));
    let gameItem = {
      name: item.node.name,
      ...
      release: item.node.release,
      platforms:  item.node.plarforms,
      genre:  item.node.genre[0].name,
      developers:  item.node.developers,
      publishers:  item.node.developers,
      inCart: false,
      selected: false,
      isHovered: false,
      isLiked: false,
      ...
      footage: item.node.footage.split(";")
  }
  newGamelist.push(gameItem)
  })
...
if (location.pathname != "/home/" && location.pathname != "/home/browse" && selectedGame === false) {
...
const handleSearchSubmit = (e) => {
  ...
  if (location.pathname != "/home/browse") {
    navigate('/home/browse');
  }
}
...
const handleSelectGame = (e) => {
  if (e.target.tagName === "BUTTON") {
    return
        ...
        navigate(`/home/games/${games[e.target.parentNode.id].surname}`);
  }
}
...
const openGamePage = (e) => {
  ...
  navigate(`/home/games/${selectedGameSurname}`);
}
...
useEffect(() => {
  ...
  } else if (location.pathname === "/home/browse") {
      ...
      <AnimatePresence exitBeforeEnter>
          <Routes key={location.pathname} location={location}>
            ...
            <Route path="/home/browse" element={<Browse 
                                              ...
                                              openGamePage={openGamePage}
                                          />} />
            <Route path="/home/games/:gameId" element={<GamePage
                                               cart={cart}
                                               cartAmount={cartAmount}
                                               handleHover={handleHover}
                                               hoverState={hoverState}
